...
from ...models.restore_automation_version_request import RestoreAutomationVersionRequest
...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/v1/automations/{id}/versions/{version_id}/restore".format(id=quote(str(id), safe=""),version_id=quote(str(version_id), safe=""),),
    }
...
def sync_detailed(
    ...
    *,
    ...
    """ Restore a workflow version
     ...
     Restore a YAML workflow automation by copying a previous snapshot into a new untagged current
    version. The source tag is left unchanged; the new HEAD appears in subsequent version lists as the
    untagged current row and cannot be promoted until you create a tagged copy. The JSON body may be
    `{}`; `message` is optional and defaults to a timestamped restore note. Agent automations return
    400. Requires a Bearer API key or a dashboard session.
    ...
    Args:
        ...
        body (RestoreAutomationVersionRequest): Optional JSON body. Send `{}` when no message is
            needed. Restore always creates a new untagged current snapshot; it does not retag the
            source version.
